...
def _extract_dataset_path(cfg: dict) -> Optional[str]:
    if not isinstance(cfg, dict):
        return None
    rl = cfg.get("rl") or {}
    if isinstance(rl, dict):
        for key in ("dataset_path_test", "dataset_path_eval", "dataset_path_train"):
            value = rl.get(key)
            if value:
                return str(value)
    ems = cfg.get("ems") or {}
    if isinstance(ems, dict):
        for key in ("inference_dataset_path", "forecast_dataset_path"):
            value = ems.get(key)
            if value:
                return str(value)
    return None
...
def _load_report_config(report_dir: Path, dataset_filter: str = "") -> dict:
    cfgs = sorted(report_dir.glob("*.yml")) + sorted(report_dir.glob("*.yaml"))
    if not cfgs and report_dir.parent.name == "test":
        run_name = re.sub(r"_\d{8}_\d{6}$", "", report_dir.name)
        candidate = report_dir.parent.parent / "test_configs" / f"{run_name}.yml"
        if candidate.exists():
            cfgs = [candidate]
    ...
    dataset_filter = (dataset_filter or "").strip().lower()
    preferred_cfg = None
    if dataset_filter:
        for cfg in cfgs:
            try:
                ...
                continue
            dataset_path = _extract_dataset_path(data or {}) or ""
            if dataset_filter in dataset_path.lower():
                preferred_cfg = cfg
                break

    cfg = preferred_cfg or cfgs[0]
    try:
        data = yaml.safe_load(cfg.read_text(encoding="utf-8"))
    except OSError:
        return {}
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot Pareto front for SOH final vs net operating cost (OPSD)."
    )
    parser.add_argument(
        "--rl-root",
        type=Path,
        default=Path("outputs/RL_REWARD_SWEEP_OPSD_1_MONTH/test"),
    ...
    parser.add_argument(
        "--dataset-contains",
        type=str,
        default="DE_KN_residential1",
        help="Only include runs whose config dataset path contains this string.",
    )
    parser.add_argument(
        "--exclude-runs",
        type=Path,
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("outputs/pareto_soh_costs_opsd.png"),
    ...
    dataset_filter = (args.dataset_contains or "").strip().lower()
    excluded_runs = set()
    ...
    for report in rl_reports + mpc_reports + rbc_reports:
        ...
        if not parsed:
            continue
        cfg = _load_report_config(report.parent, dataset_filter)
        dataset_path = _extract_dataset_path(cfg) or ""
        if dataset_filter and dataset_filter not in dataset_path.lower():
            continue
    ...
    ax.set_title("OPSD Pareto: Final SOH vs Operating Cost", fontsize=args.title_fontsize)
    ...
    if wear_points:
        ...
        ax_w.set_title("OPSD: Battery Wear vs Operating Cost", fontsize=args.title_fontsize)
    ...
    if not args.no_interactive:
        ...
        _plot_interactive(
            ...
            x_key="soh_final",
            x_label="Final SOH (%)",
            title="OPSD Pareto: Final SOH vs Operating Cost",
            auto_open=True,
        )
        _plot_interactive(
            wear_points,
            ...
            title="OPSD: Battery Wear vs Operating Cost",
            auto_open=True,
        )
